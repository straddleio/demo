...
import * as logsModule from '../../domain/logs.js';
import * as logStreamModule from '../../domain/log-stream.js';
import * as eventsModule from '../../domain/events.js';
...
describe('State Routes', () => {
  ...
  describe('GET /api/state', () => {
    it('should return current demo state', async () => {
      ...
      jest.spyOn(stateModule.stateManager, 'getState').mockReturnValue(mockState);

      const response = await request(app).get('/api/state').expect(200);

      expect(response.body).toEqual(mockState);
      expect(stateModule.stateManager.getState).toHaveBeenCalledTimes(1);
    });
    ...
    it('should return empty state when no data exists', async () => {
      ...
      jest.spyOn(stateModule.stateManager, 'getState').mockReturnValue(emptyState);
  ...
  describe('POST /api/reset', () => {
    it('should reset demo state and clear logs', async () => {
      jest.spyOn(stateModule.stateManager, 'reset').mockImplementation(() => {});
      jest.spyOn(logsModule, 'clearRequestLogs').mockImplementation(() => {});
      jest.spyOn(logStreamModule, 'clearLogStream').mockImplementation(() => {});
      jest.spyOn(eventsModule.eventBroadcaster, 'broadcast').mockImplementation(() => {});
      ...
      expect(stateModule.stateManager.reset).toHaveBeenCalledTimes(1);
      expect(logsModule.clearRequestLogs).toHaveBeenCalledTimes(1);
      expect(logStreamModule.clearLogStream).toHaveBeenCalledTimes(1);
      expect(eventsModule.eventBroadcaster.broadcast).toHaveBeenCalledWith('state:reset', {});
    });
  ...
  describe('GET /api/logs', () => {
    it('should return request logs', async () => {
      ...
      jest.spyOn(logsModule, 'getRequestLogs').mockReturnValue(mockLogs);

      const response = await request(app).get('/api/logs').expect(200);

      expect(response.body).toEqual(mockLogs);
      expect(logsModule.getRequestLogs).toHaveBeenCalledTimes(1);
    });

    it('should return empty array when no logs exist', async () => {
      jest.spyOn(logsModule, 'getRequestLogs').mockReturnValue([]);
  ...
  describe('GET /api/log-stream', () => {
    it('should return filtered log stream (only Straddle API and webhook entries)', async () => {
      ...
      jest.spyOn(logStreamModule, 'getLogStream').mockReturnValue(mockLogStream);
      ...
      expect(logStreamModule.getLogStream).toHaveBeenCalledTimes(1);
    });
  ...
  describe('GET /api/events/stream', () => {
    it('should call addClient when SSE connection is established', async () => {
      ...
      jest.spyOn(stateModule.stateManager, 'getState').mockReturnValue(emptyState);
      jest.spyOn(eventsModule.eventBroadcaster, 'addClient').mockImplementation(() => {});
      ...
      expect(eventsModule.eventBroadcaster.addClient).toHaveBeenCalled();
      expect(stateModule.stateManager.getState).toHaveBeenCalled();
